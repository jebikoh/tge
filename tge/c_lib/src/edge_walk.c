...
static int bresenhams_line(int x0, int y0, float z0, int x1, int y1, float z1,
...
static PyObject *edge_walk(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O!O!O!O!O!fffii", &PyArray_Type, &edge_pts,
                        ...
                        &PyArray_Type, &v1, &PyArray_Type, &v2, &z0, &z1, &z2,
                        &w, &h)) {
    return NULL;
  }
  ...
  int i = 0;
  i = bresenhams_line(x0, y0, z0, x1, y1, z1, edge_buf, edge_pts, edge_zs, i, w,
                      h);
  i = bresenhams_line(x1, y1, z1, x2, y2, z2, edge_buf, edge_pts, edge_zs, i, w,
                      h);
  i = bresenhams_line(x2, y2, z2, x0, y0, z0, edge_buf, edge_pts, edge_zs, i, w,
                      h);
